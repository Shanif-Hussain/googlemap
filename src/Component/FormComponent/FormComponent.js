...
export default function Form({ template, onSubmit, watchFields, validate }) {
    ...
    const renderFields = (field) => {
        let showField = field.displayRule ? _watchValues[field.displayRule.fieldName] === field.displayRule.value : true;
    ...
    const { register, handleSubmit, watch, trigger, formState: { errors }, setError, clearErrors } = useForm({
    ...
    const Merge_ArrayofwatchFields_nd_ArrayofwatchValues_to_JsonObject = (watchValues) => {
        const obj = {};
        watchFields.forEach((element, index) => {
            obj[element] = watchValues[index];
        });
        return obj;
    }

    const _watchValues = Merge_ArrayofwatchFields_nd_ArrayofwatchValues_to_JsonObject(watch(watchFields));

    validate(_watchValues, { errors, setError, clearErrors });

    // const _onSubmit = async (values) => {
    //     const result = await trigger(watchFields);
    //     if (result) {
    //         onSubmit(values);
    //     }
    // }
